...
### 5. Деплой на GitHub Pages (Автоматически)

Проект уже настроен для автоматической публикации через **GitHub Pages** с помощью GitHub Actions.

**Как запустить:**
1. Экспортируйте проект на GitHub.
2. В вашем репозитории на GitHub перейдите в **Settings** -> **Secrets and variables** -> **Actions**.
3. Нажмите **New repository secret**.
4. Добавьте секрет:
   - Name: `VITE_ROUTERAI_API_KEY`
   - Secret: `ваш_api_ключ_от_routerai`
5. Перейдите в **Settings** -> **Pages**.
6. В разделе **Build and deployment** выберите Source: **GitHub Actions**.
7. Теперь при каждом пуше в ветку `main` (или при ручном запуске в вкладке Actions) сайт будет автоматически собираться и публиковаться!

### 6. Деплой на другие платформы (Vercel, Netlify)
