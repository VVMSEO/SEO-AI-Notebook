...
export async function generateReport(notes: { project: string; refined: string; timestamp: string }[]): Promise<string> {
  try {
    ...
    Структура отчета:
    1. **Краткое резюме (Summary)**: Выделите наиболее критические задачи, важные инсайты или достижения из списка.
    2. **Детальный список задач**: Сгруппируйте все задачи по проектам.
    
    Используйте профессиональное форматирование (Markdown). Отчет должен быть на русском языке.
